...
escolas_integrais <- read.csv2('../../bases_tratadas/escolas_tempo_integral_v2.csv') # carregar a base de dados

escolas_integrais$nome_escola <- as.factor(escolas_integrais$nome_escola) # conversão de dados

escolas_integrais_agg <- escolas_integrais %>% group_by(nome_escola, ano_implantacao, .drop=FALSE) %>% summarise(n = n()) %>% na.omit() %>% complete(ano_implantacao, nome_escola, fill = list(n = 0)) %>% filter(nome_escola != '') %>% distinct()
...
```{r mr, echo=F, warning=F, message=F}

load("../../modelos/ENEM_LM.RData")
