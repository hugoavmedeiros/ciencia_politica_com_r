...
escolas_integrais_agg <- escolas_integrais %>% group_by(nome_escola, .drop=FALSE) %>% summarise(n = n()) %>% na.omit() 
...
leaflet(escolas_integrais) %>% addTiles() %>% addMarkers(~lon, ~lat, popup = ~as.character(nome_escola), label = ~as.character(nome_escola)) %>% addFullscreenControl()
